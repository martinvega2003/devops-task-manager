...
const TaskPage = ({ selectedTask, setSelectedTask, fetchTasks }) => {
  ...
  // 2 Sync local state when updatedTask changes
  useEffect(() => {
    if (!selectedTask) return;

    const sh = String(new Date(selectedTask.start_time).getHours()).padStart(2, '0');
    const sm = String(new Date(selectedTask.start_time).getMinutes()).padStart(2, '0');
    const eh = String(new Date(selectedTask.end_time).getHours()).padStart(2, '0');
    const em = String(new Date(selectedTask.end_time).getMinutes()).padStart(2, '0');
          ...
          <div className="w-full flex justify-start items-start gap-2 mt-4">
            <div className="w-full flex flex-col justify-start items-start gap-2 p-2">
              ...
              <div className="min-w-full flex flex-col gap-2">
                <div className='min-w-full flex items-center gap-2 p-4 border border-gray-300 dark:border-gray-600 rounded-2xl bg-white dark:bg-gray-700'>
                  {/* Start Time */}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          onChange={e => {
                            ...
                            if (v.length <= 2) setStartHour(v);
                          }}
                          onBlur={() => {
                            let v = startHour;
                            if (v === '') v = '00';
                            else if (v.length === 1) v = v.padStart(2, '0');
                            if (!validate('hour', v)) v = updatedTask.startTime.split(':')[0];
                            setStartHour(v);
                            setUpdatedTask({
                              ...updatedTask,
                              startTime: `${v}:${startMin}`,
                            });
                          }}
                        />
                        ...
                        <input
                          ...
                          onChange={e => {
                            const v = e.target.value.replace(/\D/g, '');
                            if (v.length <= 2) setStartMin(v);
                          }}
                          onBlur={() => {
                            let v = startMin;
                            if (v === '') v = '00';
                            else if (v.length === 1) v = v.padStart(2, '0');
                            if (!validate('min', v)) v = updatedTask.startTime.split(':')[1];
                            setStartMin(v);
                            setUpdatedTask({
                              ...updatedTask,
                              startTime: `${startHour}:${v}`,
                            });
                          }}
                        />
                      </div>
                    ...
                    {!isEditing ? (
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          onChange={e => {
                            const v = e.target.value.replace(/\D/g, '');
                            if (v.length <= 2) setEndHour(v);
                          }}
                          onBlur={() => {
                            let v = endHour;
                            if (v === '') v = '00';
                            else if (v.length === 1) v = v.padStart(2, '0');
                            if (!validate('hour', v)) v = updatedTask.endTime.split(':')[0];
                            setEndHour(v);
                            setUpdatedTask({
                              ...updatedTask,
                              endTime: `${v}:${endMin}`,
                        ...
                        <input
                          ...
                          onChange={e => {
                            const v = e.target.value.replace(/\D/g, '');
                            if (v.length <= 2) setEndMin(v);
                          }}
                          onBlur={() => {
                            let v = endMin;
                            if (v === '') v = '00';
                            else if (v.length === 1) v = v.padStart(2, '0');
                            if (!validate('min', v)) v = updatedTask.endTime.split(':')[1];
                            setEndMin(v);
                            setUpdatedTask({
                              ...updatedTask,
                              endTime: `${endHour}:${v}`,
